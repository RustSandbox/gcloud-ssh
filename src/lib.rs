...
mod tests {
    ...
    /// Test that application title is not empty
    #[test]
    fn test_app_title_not_empty() {
        assert!(!config::APP_TITLE.is_empty());
    }

    /// Test formatting of VM list items
    #[test]
    fn test_vm_list_item_format() {
        let formatted = format_vm_list_item(0, "test-vm", "us-central1-a", Some("192.168.1.1"));
        assert!(formatted.contains("test-vm"));
        assert!(formatted.contains("us-central1-a"));
        assert!(formatted.contains("192.168.1.1"));
    }

    /// Test SSH command box formatting
    #[test]
    fn test_ssh_command_box() {
        let command = "ssh user@host";
        let box_output = banner::ssh_command_box(command);
        assert!(box_output.contains(command));
        assert!(box_output.contains("┌")); // Contains top border
        assert!(box_output.contains("┘")); // Contains bottom-right corner
    }
}
